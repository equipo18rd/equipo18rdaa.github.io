document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('nav ul li a').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            const targetId = this.getAttribute('href'); // Obtiene el ID de la sección a la que apunta el enlace.
            const targetSection = document.querySelector(targetId); // Selecciona la sección de destino.

            if (targetSection) {
                // Desplaza la vista a la sección de destino con un efecto suave.
    ...
    // Verifica si el formulario existe antes de intentar añadir un event listener.
    if (feedbackForm) {
        feedbackForm.addEventListener('submit', (event) => {
            event.preventDefault(); // Evita el envío de formulario por defecto (recarga la página).
            ...
            // Muestra los datos recopilados en la consola del navegador.
    ...
    // 3. Contenido Dinámico para "Botones de Huella"
    // (Asumiendo que agregarás estos botones en tu HTML con la clase 'paw-button' y un 'data-info')
    ...
    pawButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (buttonContent[infoKey]) {
                infoDisplay.innerHTML = buttonContent[infoKey]; // Cambia el contenido del div #info-display.
                infoDisplay.scrollIntoView({ behavior: 'smooth', block: 'start' }); // Desplazamiento suave a la sección.
            } else {
